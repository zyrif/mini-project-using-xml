...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if($file){
		...
		if($idcorrect == true && $passcorrect == true && $namecorrect == true && $emailcorrect == true && $usertypecorrect == true){
			...
			if($users = simplexml_load_file("record.xml")){
				$users->addChild("user");
				$users->user->addChild("id", "$id");
				$users->user->addChild("password", "$password");
				$users->user->addChild("name", "$name");
				$users->user->addChild("email", "$email");
				$users->user->addChild("usertype", "$usertype");
			}
			...
			if($user->asXML("record.xml"))
				echo "Successful";
			else
				echo "Xml saving failed";
